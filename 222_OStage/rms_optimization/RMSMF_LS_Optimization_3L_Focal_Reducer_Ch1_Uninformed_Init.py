"""
Direct RMS control experiment from an uninformed initialization.

This script preserves the optical architecture and RMS optimization settings
of the original channel-1 script, but bypasses the physically grounded
SPD/PGMF initialization. By default, each lens is initialized with equal
front/back radii drawn reproducibly from a log-uniform distribution.

Edit INIT_MODE, RANDOM_SEED, radius limits, or PAIR_RADII_MM in the
"Uninformed initialization settings" section to generate additional controls.
"""
...
# ============================================================
#
# This control experiment bypasses the paraxial/third-order SPD and the
# intermediate PGMF stage. The same three-lens architecture, glasses,
# thicknesses, air gaps, RMS merit function, fields, wavelengths, bounds,
# and solver settings are retained.
#
# Available modes:
#   "equal_pair_fixed"  -> R1=R2, R3=R4, R5=R6 using PAIR_RADII_MM
#   "equal_pair_random" -> reproducible random equal-radius pairs
#   "fully_random"      -> six independent reproducible random radii
#
INIT_MODE = "equal_pair_random"
RANDOM_SEED = 20260716

# Used only by equal_pair_fixed.
PAIR_RADII_MM = np.array([500.0, -500.0, 500.0], dtype=float)

# Used by random modes. Magnitudes are sampled log-uniformly.
RADIUS_ABS_MIN_MM = 100.0
RADIUS_ABS_MAX_MM = 2000.0

# For equal_pair_random, these signs define the three lens-pair radii.
# They preserve a reproducible + / - / + sequence without using the SPD
# power allocation. Set PAIR_SIGNS = None to draw signs randomly.
PAIR_SIGNS = np.array([1.0, -1.0, 1.0], dtype=float)


def sample_log_uniform_radii(rng, n, rmin, rmax, signs=None):
    """Sample signed radii with log-uniform absolute magnitude."""
    if rmin <= 0 or rmax <= rmin:
        raise ValueError("Require 0 < rmin < rmax.")

    magnitudes = np.exp(rng.uniform(np.log(rmin), np.log(rmax), size=n))
    ...
    if signs is None:
        ...
        if signs.shape != (n,):
            raise ValueError(f"Expected {n} signs, received shape {signs.shape}.")
        signs = np.where(signs >= 0.0, 1.0, -1.0)

    return signs * magnitudes


def build_uninformed_initial_radii():
    """Return six radii for the selected control initialization."""
    rng = np.random.default_rng(RANDOM_SEED)

    if INIT_MODE == "equal_pair_fixed":
        pair_radii = np.asarray(PAIR_RADII_MM, dtype=float)
        if pair_radii.shape != (3,):
            raise ValueError("PAIR_RADII_MM must contain exactly three values.")
        radii = np.repeat(pair_radii, 2)

    elif INIT_MODE == "equal_pair_random":
        pair_radii = sample_log_uniform_radii(
            rng,
            n=3,
            rmin=RADIUS_ABS_MIN_MM,
            rmax=RADIUS_ABS_MAX_MM,
            signs=PAIR_SIGNS,
        )
        radii = np.repeat(pair_radii, 2)

    elif INIT_MODE == "fully_random":
        radii = sample_log_uniform_radii(
            rng,
            n=6,
            rmin=RADIUS_ABS_MIN_MM,
            rmax=RADIUS_ABS_MAX_MM,
        ...
        raise ValueError(
            "INIT_MODE must be 'equal_pair_fixed', "
            "'equal_pair_random', or 'fully_random'."
        )

    if not np.all(np.isfinite(radii)):
        raise ValueError("Generated non-finite initial radii.")

    return radii.astype(float)


R = build_uninformed_initial_radii()
...
d_5 = 109.45220040094777

## Seed Wroking Distance Glass S-FPL51 & F2HT
# d_5 = 109.68393688854448
...
# Control initialization generated without paraxial power allocation,
# third-order aberration balancing, or PGMF refinement.
R1_initial, R2_initial, R3_initial, R4_initial, R5_initial, R6_initial = R

# Keep the same mechanical working-distance seed used in the original
# direct-RMS script so that the control isolates the effect of the radii.
# d_5 was defined above for the selected glass combination.

init_tag = f"{INIT_MODE}_seed{RANDOM_SEED}"
...
EFFL_INITIAL = Telescope_f85_FR.EFFL 
...
# Telescope_f85_FR.SDT[8].Thickness = 1.092475999398023E+002
...
file_path = (
    output_dir
    / f"RMSMF_LS_Parameters_{init_tag}_{L1a.Glass}_{L2a.Glass}_{L3a.Glass}_Ch1.txt"
)

with open(file_path, "w", encoding="utf-8") as f:

    f.write(
        f"RMSMF_LS_Parameters_{init_tag}_{L1a.Glass}_{L2a.Glass}_{L3a.Glass}_Ch1= [\n"
    )
...
name_save = f"RMS_Three_Lens_LS_{init_tag}_{L1a.Glass}_{L2a.Glass}_{L1a.Glass}_Ch1"
...
List_Radius, meta = run_spots_for_fields(
    ...
    box_include_airy=False, save_dir = r'figures\SPT_Diagrams\Ch1')


EE_Example_information = plot_all_EE_for_fields(
                            Pup, Rays, Field_ccd, RW, save_dir=r'figures\EE_Diagrams\Ch1',
                            show_r50 = True, save = True,  show=True,
                            filename=f"EE_RMS_Three_Lens_LS_{init_tag}_{L1a.Glass}_{L2a.Glass}_{L1a.Glass}_Ch1.pdf"
                            )   
