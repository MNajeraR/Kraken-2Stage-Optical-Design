...
class Optimizer():
    ...
    def Set_RcValues(self, V):
        ...
        # print(Values_CH[0], Values_CH[1], Values_CH[2], Values_CH[3])
        # Apply the changes
        self.system.SetData()
        ...
        self.valueChrom = self.Aberration.Chromatic(1, [0., 0.])[1]
        self.valueShp = self.Aberration.Spheric(1, [0., 0.])[1]
        self.valueComa = self.Aberration.Coma(1, [0., -self.Field])[1]
        
        if np.isnan(self.valueComa):
            self.valueComa = 1e6
        
        self.valueAst = self.Aberration.Astigmatism(1, [0., -self.Field])[1]
        
        
        print(w_1*self.valueChrom, w_2*self.valueShp, w_3*self.valueComa, w_4*self.valueAst)
        
        # Compute the merit function
        self.value = w_1*self.valueChrom**2 + w_2*self.valueShp**2+w_3*self.valueComa**2+w_4*self.valueAst**2
       
        # Evaluate the final merit function with EFFL deviation
        self.D_EFFL = np.abs(self.system.EFFL - self.EFFL_Tr)
        
        self.merit_fun = self.value + w_5*self.D_EFFL**2
        ...
        return w_1*self.valueChrom, w_2*self.valueShp, w_3*self.valueComa, w_4*self.valueAst, w_5*self.D_EFFL
