...
"""
======================================
  Script: Monte Carlo Optimization Setup
======================================
...
Description:
------------
This script configures and optimizes an optical system using KrakenOS, based on 
a starting point derived from first- and third-order analysis. It employs a 
Monte Carlo approach with Gaussian sampling centered on an initial design to 
explore the solution space. The optimization is guided by a physically grounded 
merit function incorporating Fermat’s principle, coma cancellation, Coddington 
equations, and chromatic path equalization. Final performance is 
evaluated through metrics such as GEO radius, RMS radius, EFFL deviation, 
convergence time  and spot diagram visualization.

Dependencies:
-------------
- NumPy: For numerical operations and Gaussian sampling.
- SciPy: For numerical routines (used indirectly).
- KrakenOS (Kos): Optical simulation environment.
- MOS_Class: Custom module implementing the merit function, aberration 
             evaluation, the `Optimizer` class, and the `Random_RO` Monte Carlo sampler.
...
# # Third Order
# R_1 = 88.26054458103616   + R_1*0.2
# R_2 = -383.5298465374939  + R_2*0.2
# R_3 = -139.29006826264188 + R_3*0.2
# R_4 = 292.84629048211497  + R_4*0.2
...
print(f"Coma Aberration: {Aberration.Coma(1, Field_ccd)[1]*(1000/W):.2f}")
print(f"Astigmatism Aberration: {Aberration.Astigmatism(1, Field_ccd)[1]*(1000/W):.2f}")
...
Monte_Carlo_Result = Optimizer([Telescope_f85_FR,Rays])
...
centers = [R_1, R_2, R_3, R_4, d_4]
# ro = Random_RO(1000, centers)

# Define the search limits for each variable
Delta_search= 0.20

opt = Random_RO(num_range=1000, centers=centers)

limits = [(R_1 - R_1*Delta_search, R_1 + R_1*Delta_search), 
          (R_2 + R_2*Delta_search, R_2 - R_2*Delta_search),
          (R_3 + R_3*Delta_search, R_3 - R_3*Delta_search), 
          (R_4 - R_4*Delta_search, R_4 + R_4*Delta_search),
          (d_4 - d_4*Delta_search,d_4 + d_4*Delta_search)]
...
print(f"ΔWorking distance (d4): {np.abs(d4+deltaZ - d_4):.2f} mm")
print("======================================")
...
name_save = f"PGMF_Monte_Carlo_{L1a.Glass}_{L2a.Glass}"
...
EE_Example_information = plot_all_EE_for_fields(
                            ...
                            filename=f"EE_Monte_Carlo_{L1a.Glass}_{L2a.Glass}_fields.pdf"
                            )   
