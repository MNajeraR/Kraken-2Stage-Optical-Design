...
GA_Result = Optimizer([Telescope_f85_FR, Rays])
...
limits = [
    {'low': R_1 - R_1*Delta_search, 'high': R_1 + R_1*Delta_search},
    {'low': R_2 + R_2*Delta_search, 'high': R_2 - R_2*Delta_search},
    {'low': R_3 + R_3*Delta_search, 'high': R_3 - R_3*Delta_search},
    {'low': R_4 - R_4*Delta_search, 'high': R_4 + R_4*Delta_search},
    {'low': d_4 - d_4*Delta_search,  'high': d_4 + d_4*Delta_search}
]

# Genetic operators and configuration
parent_selection_type = "random"          # Steady-state selection
...
# Plot individual Spot Diagrams 
name_save = f"PGMF_GA_{L1a.Glass}_{L2a.Glass}"
...
EE_Example_information = plot_all_EE_for_fields(
                            ...
                            filename=f"EE_GA_{L1a.Glass}_{L2a.Glass}_fields.pdf"
                            )   
