...
ABC_Result = Three_Lens_Optimizer([Telescope_f85_FR,Rays])
...
search_boundaries = (
    [R1_initial - R1_initial*Delta_search, R2_initial - R2_initial*Delta_search,
     R3_initial + R3_initial*Delta_search, R4_initial - R4_initial*Delta_search, 
     R5_initial - R5_initial*Delta_search, R6_initial + R6_initial*Delta_search, 
     d_5 - d_5*Delta_search], 
    [R1_initial + R1_initial*Delta_search, R2_initial + R2_initial*Delta_search,
     R3_initial - R3_initial*Delta_search, R4_initial + R4_initial*Delta_search, 
     R5_initial + R5_initial*Delta_search, R6_initial - R6_initial*Delta_search, 
     d_5 + d_5*Delta_search]
...
alg.performFullOptimisation(max_iteration=200)
...
file_path = os.path.join(folder_name,f"PGMF_ABC_Parameters_{L1a.Glass}_{L2a.Glass}_{L3a.Glass}.txt")

with open(file_path, "w", encoding="utf-8") as f:
    f.write(f"PGMF_ABC_Parameters_{L1a.Glass}_{L2a.Glass}_{L3a.Glass}= [\n")
...
name_save = f"PGMF_Three_Lens_ABC_{L1a.Glass}_{L2a.Glass}_{L1a.Glass}"
...
EE_Example_information = plot_all_EE_for_fields(
                            ...
                            filename=f"EE_PGMF_Three_Lens_ABC_{L1a.Glass}_{L2a.Glass}_{L1a.Glass}.pdf"
                            )   
